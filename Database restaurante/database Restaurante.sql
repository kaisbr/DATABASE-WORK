...
show tables;
-- pesquisa do cliente que mais fez pedidos
SELECT 
...
-- pesquisa do cliente que mais gastou
SELECT 
    c.nome_cliente AS NomeCliente,
    SUM(p.preco_unitario_prato * CAST(ped.quantidade_pedido AS DECIMAL)) AS TotalGasto
FROM 
    tb_cliente c
JOIN 
    tb_mesa m ON c.id_cliente = m.id_cliente
JOIN 
    tb_pedido ped ON m.codigo_mesa = ped.codigo_mesa
JOIN 
    tb_prato p ON ped.codigo_prato = p.codigo_prato
GROUP BY 
    c.nome_cliente
ORDER BY 
    TotalGasto DESC
